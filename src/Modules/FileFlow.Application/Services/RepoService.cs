...
internal class RepoService(ILogger<RepoService> logger) : IRepoService
{
    ...
    public async Task CommitAsync(string message)
    {
        ...
        commitInfoBuilder.Append($"Commit from {DateTime.Now.ToString("dddd, dd MMMM yyyy HH:mm:ss")} - '{message}' - {changes.Added.Count} insertions, {changes.Removed.Count} deletions.");
